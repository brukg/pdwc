...
int main(int argc, char** argv){
  ...
  printf("%s", msg);
  printf("\rSpeed -> %f and yaw rate->  %f | Last command:", speed, turn);
  ...
  while(rclcpp::ok()){
    ...
    else if (speedBindings.count(key) == 1)
    {
      ...
      printf("\rSpeed is %f and turn is %f | Last command: %c ", speed, turn, key);
    }
    ...
    { if (key=='s'||key=='S'){
      ...
      z = 0;
      th = 0;
      printf("\r\t\tRobot Stopped..!! ");
      printf("\rSpeed %f\tturn %f | Last command: %c   ", speed*x, turn*th, key);
    }
      ...
      else if (key == '\x03')
      {
        ...
        printf("\r Speed %f\tturn %f | Invalid command! %c", speed*x, turn*th, key);
    }
